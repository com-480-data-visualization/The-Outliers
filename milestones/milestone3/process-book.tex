...
\graphicspath{{../img/wireframes/}{../img/}{../../img/}}

% Small caps eyebrow label used at the top of each page
\newcommand{\eyebrow}[1]{{\footnotesize\textcolor{accentBlue}{\textsc{#1}}}}

% Combined page header: eyebrow line + bold title + a thin two-tone accent
% rule that anchors the visual top of every page. Usage:
%   \pageheader{04}{Designing the narrative}{A four-chapter arc, Freytag inside}
\newcommand{\pageheader}[3]{%
  {\eyebrow{#1 \quad\textbullet\quad #2}}\\[3pt]%
  {\Large\bfseries #3}\\[3pt]%
  {\color{accentBlue}\rule{4em}{1.5pt}}\hspace{1pt}%
  {\color{accentBlue!28}\rule{\dimexpr\linewidth-4em-1pt\relax}{0.5pt}}\\[4pt]%
}

% Slim left-bordered callout. Lighter than a full tcolorbox; used for one-line
% findings that deserve to be lifted out of the prose without dominating.
\newcommand{\callout}[2]{%
  \vspace{2pt}%
  \begin{tcolorbox}[colback=accentBlue!4!white, colframe=accentBlue!60!white,
    boxrule=0pt, leftrule=2.5pt, arc=1.5pt,
    left=9pt, right=8pt, top=3pt, bottom=3pt]%
  \footnotesize\textbf{\textcolor{accentBlue}{#1}}\quad#2%
  \end{tcolorbox}%
  \vspace{1pt}%
}

% Inline color chip with white-on-color label. Used to render the palette
% as a row of swatches rather than coloured inline text.
\newcommand{\hexchip}[2]{%
  \colorbox{#1}{\color{white}\strut\,\footnotesize\textbf{#2}\,}%
}

% Subsection heading rendered as a small rounded accent-blue pill. Replaces
% \subhead{...} everywhere in the document so every in-page heading reads
% as a designed element instead of plain bold text.
\newcommand{\subhead}[1]{%
  \vspace{4pt}%
  \noindent\tcbox[
    on line,
    colback=accentBlue!10!white, colframe=accentBlue,
    boxrule=0.3pt, arc=2.5pt,
    left=7pt, right=7pt, top=0.5pt, bottom=0.5pt,
  ]{\small\bfseries\color{accentBlue!85!black}#1}\\[2pt]%
}
...
% Hero screenshot
\begin{center}
\includegraphics[height=8cm,keepaspectratio]{hero_cover.png}
\end{center}
...
\pageheader{02}{From idea to thesis}{Where we started, where we landed}

\vspace{2pt}

The M1 problematic was deliberately broad: ``orbital space is becoming infrastructure, and that infrastructure is invisible to most people.'' We picked the UCS Satellite Database because it combines what a satellite is, who operates it, what it is for, and where it sits in a single curated file. Three things changed between the M1 sketches and the final site: the thesis sharpened, the chapter count grew from three to four, and the 3D component was promoted from a Chapter~3 prop to the page's hero. The three M1 wireframes below are shown next to their final-site counterparts so the evolution is visible at a glance; a fourth element (the 3D hero) had no M1 sketch and is captured separately on page~1.

\vspace{4pt}

\textbf{Wireframe 1 $\to$ Chapter 1.} Kept almost unchanged: five scroll-locked steps still anchor the timeline at 1974, 1998, 2018, 2021, 2023. The single addition is a stacked-area panel underneath so the reader sees that the post-2019 surge is overwhelmingly Communications (Starlink and OneWeb).

\vspace{2pt}
\begin{center}
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{M1 wireframe}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{01_timeline.png}
\end{minipage}\hfill%
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{Final site}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{final_timeline.png}
\end{minipage}
\end{center}

\textbf{Wireframe 2 $\to$ Chapter 2.} The Lorenz + Gini 0.862 callout survived as drawn (the wireframe used a placeholder ranking of operators; the final swaps in the real top-10 with SpaceX dominating). Around it we added a world choropleth, a country bar, an operator deep-dive drawer, and a country$\times$purpose specialization heatmap.

\vspace{2pt}
\begin{center}
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{M1 wireframe}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{05_concentration.png}
\end{minipage}\hfill%
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{Final site}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{final_concentration.png}
\end{minipage}
\end{center}

\textbf{Wireframe 3 $\to$ Chapter 3.} The heatmap stayed exactly as designed because it was already doing what a heatmap should: making a 2D conditional distribution legible. The static globe sketch was replaced by a Globe.gl viewport with three composed filters (orbit, year, mega-constellation) and a satellite-name search with a pulsing-ring spotlight; that interactive surface is documented on page~6.

\vspace{2pt}
\begin{center}
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{M1 wireframe}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{04_purpose.png}
\end{minipage}\hfill%
\begin{minipage}[c]{0.46\textwidth}\centering
{\scriptsize\textcolor{textSec}{\textsc{Final site}}}\\[1pt]
\includegraphics[height=3.6cm,keepaspectratio]{final_purpose.png}
\end{minipage}
\end{center}

\textbf{What wasn't in M1.} A 3D hero (page~1) and a fourth chapter. M1's narrative ended on present-day crowding, but the 2030 projections are what give the story stakes, so Chapter~4 (``The Next Decade'') was added late. The hero scene was driven by the screencast: a cinematic establishing shot in the first ten seconds lets us say more, faster, than any chart could.
...
\pageheader{03}{Data and exploratory analysis}{6{,}713 satellites, cleaned and aggregated}
...
\subhead{The picture that justified the entire project}

\noindent
\begin{minipage}[t]{0.48\textwidth}
\vspace{2pt}
The orbit donut from Chapter 3 (right) shows why the four-bucket classification is meaningful, not arbitrary: the population concentrates sharply. LEO holds \textbf{5{,}940 satellites (88.4\%)}, almost an order of magnitude more than every other orbit combined. GEO accounts for \textbf{580 (8.6\%)}, MEO for \textbf{142 (2.1\%)}, and the highly-elliptical category for \textbf{51 (0.8\%)}. The buckets correspond to physically distinct altitude bands (LEO below 2{,}000 km, MEO between 2{,}000 and $\approx 36{,}000$ km, GEO at $\approx 36{,}000$ km, Elliptical with high eccentricity) where satellites cluster for reasons of orbital mechanics, launch economics, and mission requirements. This concentration is the structural foundation of every other chart on the site: the purpose $\times$ orbit heatmap, the time-slider history, the bloom-ringed shells in the hero.
\end{minipage}\hfill%
\begin{minipage}[t]{0.48\textwidth}
\vspace{0pt}
\includegraphics[width=0.95\textwidth]{orbit-donut.png}
\end{minipage}

\vspace{4pt}

\subhead{Output schema}
...
\subhead{Freytag's pyramid mapped to chapters}
...
\subhead{Why these chart types, specifically}
...
\pageheader{05}{Visual design decisions}{A dark space, four hues, two typefaces}
...
Four orbit-class hues form the spine:

\vspace{2pt}
\begin{center}
\hexchip{accentBlue}{LEO}\,\texttt{\footnotesize\#1A73E8}\quad
\hexchip{accentOrange}{MEO}\,\texttt{\footnotesize\#E8710A}\quad
\hexchip{accentGreen}{GEO}\,\texttt{\footnotesize\#34A853}\quad
\hexchip{accentPurple}{Elliptical}\,\texttt{\footnotesize\#9334E6}
\end{center}
\vspace{2pt}

The same hex codes are used identically in the D3 charts, the Three.js hero scene's GLSL shader, and the Chapter 3 globe.gl points layer, defined once in \texttt{prototype/js/charts/shared.js} and imported wherever a chart needs the palette. The dual role (orbit class \textit{and} accent / highlight) is the one place where the palette had to flex: in the country bar chart, the USA bar uses the orange accent because it is the dominant entry, which conflicts with MEO's orange. We accepted the conflict because the rule ``the dominant entry is orange'' generalizes cleanly across charts (it is also the highlight for active constellation isolator buttons), whereas a sixth color would have diluted the orbit-class palette.
...
\subhead{Library choices, with rationale}
...
\pageheader{06}{Interactions and challenges}{Six interactions, three problems that nearly bit us}

\vspace{2pt}

\subhead{The interactions, in the order the reader meets them}
...
\subhead{Challenges, with what we did about each}
...
\pageheader{07}{Technical implementation}{Build, ship, test}
...
\subhead{Build pipeline and testing}
...
\pageheader{08}{Peer assessment and reflection}{Who did what, and what we would do differently}
...
\arrayrulecolor{accentBlue!50!white}
{\small
\begin{tabularx}{\textwidth}{@{} >{\columncolor{accentBlue!8!white}}l X >{\columncolor{accentBlue!5!white}}r @{}}
\arrayrulecolor{accentBlue}\toprule
\rowcolor{accentBlue!12!white}
\textbf{\textcolor{accentBlue!85!black}{Member}} & \textbf{\textcolor{accentBlue!85!black}{Primary contributions}} & \textbf{\textcolor{accentBlue!85!black}{Share}}\\
\arrayrulecolor{accentBlue!50!white}\midrule
\textbf{Kevin Abou Jaoude} \newline {\footnotesize\textcolor{textSec}{358300}}
& Chapter 3 interactive 3D globe: year-filter slider with play-history sweep, mega-constellation isolator with linked-view dispatch, satellite-name search with pulsing-ring spotlight, and the linked-country highlighting that ties the isolator to both the Chapter 2 country bar chart and the Chapter 2 world choropleth. Data pipeline expansion in \texttt{build\_data.py}: new aggregations for cumulative-purpose-by-year, country counts, country$\times$purpose matrix, operator deep-dive details, plus the constellation and launch-year fields on the per-satellite list (23 tests passing throughout). Repo hygiene: README v2, MIT \texttt{LICENSE}, expanded \texttt{.gitignore}. Process-book authoring and structure. & \textbf{1/3}\\
\addlinespace[2pt]
\textbf{Youssef Dib} \newline {\footnotesize\textcolor{textSec}{339964}}
& Chapter 2 world choropleth (D3 + TopoJSON, Equal-Earth projection, log color scale, country-name normalization, sphere outline, hover tooltip with global share, linked-view highlight from the Chapter 3 constellation isolator). Cinematic hero intro: three-act poster sequence, radial-iris vignette wipe, latched phase state machine that consumes wheel/touch input without leaking page scroll. Fixed top-right chapter navigator (open / close / active-chapter tracking via IntersectionObserver). Hero 3D scene polish: \texttt{UnrealBloomPass} post-processing, custom GLSL shader for soft-glow satellite sprites, orbit-class color tuning so each colored shell reads against the LEO mass. Globe.gl robustness: Ctrl-to-zoom gating with on-screen hint, \texttt{ResizeObserver} pattern, atmosphere alpha fix, scrollbar hiding so the layout never shifts. & \textbf{1/3}\\
\addlinespace[2pt]
\textbf{Mark Nabbout} \newline {\footnotesize\textcolor{textSec}{358312}}
& Chapter 1 stacked area chart (purpose over time) with the post-2019 Starlink-era annotation. Chapter 2 country$\times$purpose specialization heatmap with floating info card. Chapter 2 top-operators bar chart and full deep-dive drawer (launch-timeline sparkline, orbit and purpose mini-bars, KPI tiles, focus-trapped \texttt{role="dialog"}). Chapter 4 (``The Next Decade'') projection chart with the citation block. Lenis smooth-scroll integration and the chapter-entrance fade-in observer. Site-wide accessibility pass (\texttt{aria-label}s, \texttt{noscript} fallback, \texttt{prefers-reduced-motion} guards, Open Graph meta). & \textbf{1/3}\\
\arrayrulecolor{accentBlue}\bottomrule
\end{tabularx}
}
...
\subhead{What we would do differently with more time}
